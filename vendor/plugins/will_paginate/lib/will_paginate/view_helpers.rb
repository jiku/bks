module WillPaginate
  ...
  module ViewHelpers
    ...
    def will_paginate(entries = @entries, options = {})
      ...
      return if entries.class != WillPaginate::Collection
      ...
      if total_pages > 1
        ...
        # In links, show # of items as well...
        div_content = links.join(options.delete(:separator))
        div_content << "<br/>" 
        end_item = entries.current_page*entries.per_page
        start_item = (end_item-entries.per_page)+1
        div_content << content_tag(:span, "(Showing items #{start_item}-#{end_item} of #{entries.total_entries} total)", :class => 'info')
        content_tag :div, div_content, options
