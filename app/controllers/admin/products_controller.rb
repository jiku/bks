class Admin::ProductsController < Admin::BaseController
  ...
  before_filter :set_manufacturers, :set_tags
  ...
  # Shows all manufacturers in system and lets user list products by manufacturer.
  ...
  def list_by_manufacturers
    ...
    @manufacturer = Manufacturer.find(:first, :conditions => ["id=?", @viewing_by])
    if @manufacturer == nil then
		  redirect_to :action => 'list'
		  return
    end

    @title = "Product List For Manufacturer - '#{@manufacturer.name}'"
    ...
    @products = @manufacturer.products.paginate(
      :order => "name ASC",
      :page => params[:page],
      :per_page => 30
    )
  ...
  def destroy
	...
	# Called when updating Manufacturers from the product edit page
	# Returns the rendered partial for our Manufacturer list
...
private
  # Sets the manufacturers and tags instance variable
  #
